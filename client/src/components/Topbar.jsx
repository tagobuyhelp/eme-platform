...
function Topbar({ role, toggleSidebar, toggleMobileDrawer, theme, toggleTheme, profilePhoto }) {
  ...
  return (
    <>
      <header className={`sticky top-0 z-30 flex h-[60px] w-full items-center justify-between px-6 transition-colors duration-300 border-b ${
        ...
        <div className="flex items-center gap-4">
          ...
          <button 
            ...
            className={`lg:hidden flex h-10 w-10 items-center justify-center rounded-lg ${theme === "dark" ? "hover:bg-slate-800 text-white" : "hover:bg-slate-100 text-slate-600"}`}
          >
          ...
          <button 
            ...
            className={`hidden lg:flex h-10 w-10 items-center justify-center rounded-lg ${theme === "dark" ? "hover:bg-slate-800 text-white" : "hover:bg-slate-100 text-slate-600"}`}
          >
            <span className="material-icons">menu_open</span>
          </button>

          <h2 className={`font-heading text-lg font-extrabold tracking-tight leading-none ${theme === "dark" ? "text-white" : "text-slate-900"}`}>
            {pageTitle}
          </h2>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          {role === "admin" && (
            <div className="relative">
              ...
              >
                ...
                {unreadCount > 0 && (
                  <span className="absolute top-1.5 right-2 h-2.5 w-2.5 rounded-full bg-rose-500 ring-2 ring-white animate-pulse"></span>
                )}
              </button>

              {showDropdown && (
                <div className="absolute right-0 mt-3 w-80 rounded-2xl bg-white shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden z-50">
                  <div className="px-4 py-3 border-b border-slate-50 bg-slate-50 flex justify-between items-center">
                    <h3 className="font-bold text-slate-800 text-sm">Notifications</h3>
                    {notifications.length > 0 && (
                      <button onClick={() => setNotifications([])} className="text-xs text-indigo-600 hover:text-indigo-800 font-bold">Clear All</button>
                    )}
                  </div>
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:block text-right">
              ...
              <p className={`text-[10px] uppercase tracking-widest truncate ${theme === "dark" ? "text-slate-400" : "text-slate-500"}`}>
                {role === "admin" ? "Administrator" : "Student"}
              </p>
            </div>
            <div className="h-9 w-9 rounded-full bg-indigo-600 flex items-center justify-center text-white font-bold shadow-lg shadow-indigo-600/20 overflow-hidden">
